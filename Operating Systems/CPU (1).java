...
public class CPU {
    ...
    public static int[] registers; // 16 registers (R0 to R15)
    ...
    public void fetch() {
        if (programCounter <= currentPCB.getEndAddress()) {
            currentInstruction = memory.readFromRAM(programCounter);
            outputWriter.printf("Fetching instruction at PC=0x%04X: 0x%08X%n", programCounter * 4, currentInstruction);
            // Debug Statement
            System.out.printf("CPU: Fetched instruction 0x%08X from address 0x%04X%n", currentInstruction, programCounter);
            programCounter++;
        } else {
            running = false;
        }
    ...
    public void executeRD() {
        if (instrType == 0b11) { // Input and Output Instruction Format
            int data;
            if (reg2 == 0) {
                // Direct addressing
                data = memory.readFromRAM(address);
                outputWriter.printf("Executing RD: R%d = MEM[0x%X]%n", reg1, address);
            } else {
                // Indirect addressing
                int effectiveAddress = registers[reg2] + address;
                data = memory.readFromRAM(effectiveAddress);
                outputWriter.printf("Executing RD: R%d = MEM[R%d + 0x%X] = MEM[0x%X]%n", reg1, reg2, address, effectiveAddress);
            }
            registers[reg1] = data;
        } else {
    ...
    public void executeWR() {
        if (instrType == 0b11) { // Input and Output Instruction Format
            if (reg2 == 0) {
                // Direct addressing
                memory.writeToRAM(address, registers[reg1]);
                outputWriter.printf("Executing WR: MEM[0x%X] = R%d%n", address, reg1);
            } else {
                // Indirect addressing
                int effectiveAddress = registers[reg2] + address;
                memory.writeToRAM(effectiveAddress, registers[reg1]);
                outputWriter.printf("Executing WR: MEM[R%d + 0x%X] = MEM[0x%X] = R%d%n", reg2, address, effectiveAddress, reg1);
            }
        } else {
    ...
    public void executeST() {
        if (instrType == 0b11 || instrType == 0b01) { // Input/Output or Immediate Format
            ...
            memory.writeToRAM(effectiveAddress, registers[reg1]);
            outputWriter.printf("Executing ST: MEM[0x%X] = R%d%n", effectiveAddress, reg1);
        } else {
